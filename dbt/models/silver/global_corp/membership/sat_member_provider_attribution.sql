...
SELECT
    hash_key,
        ...
        TRY_CAST((atrributed_provider_npi) AS VARCHAR) AS attributed_provider_npi,
        TRY_CAST((CAST(attributed_provider_contract_id AS TEXT)) AS VARCHAR) AS attributed_provider_contract_id,
        TRY_CAST((member_provider_pcp_begin_date) AS DATE) AS pcp_attribution_date,
        TRY_CAST((managed_care_organization_contract_number) AS VARCHAR) AS mco_contract_number,
