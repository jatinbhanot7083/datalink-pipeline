...
SELECT
    hash_key,
        TRY_CAST(CAST((provider_contracted_entity_name) AS VARCHAR) AS VARCHAR) AS contracted_entity_name,
        TRY_CAST(CAST((CAST(refresh_date AS DATE)) AS VARCHAR) AS DATE) AS data_refresh_date,
    _load_dt,
    _record_source,
    _batch_id,
    SHA2_HEX(CONCAT_WS('|', COALESCE(CAST((provider_contracted_entity_name) AS VARCHAR), ''), COALESCE(CAST((CAST(refresh_date AS DATE)) AS VARCHAR), '')), 256) AS _hash_diff,
...
{% if is_incremental() %}
  AND (SHA2_HEX(CONCAT_WS('|', COALESCE(CAST((provider_contracted_entity_name) AS VARCHAR), ''), COALESCE(CAST((CAST(refresh_date AS DATE)) AS VARCHAR), '')), 256)) NOT IN (
