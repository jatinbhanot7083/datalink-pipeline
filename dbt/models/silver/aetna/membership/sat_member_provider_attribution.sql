...
SELECT
    hash_key,
        ...
        TRY_CAST(CAST((attributed_provider_contract_id) AS VARCHAR) AS DECIMAL(20, 4)) AS attributed_provider_contract_id,
    ...
    _record_source,
    _batch_id,
    SHA2_HEX(CONCAT_WS('|', COALESCE(CAST((attributed_provider_id) AS VARCHAR), ''), COALESCE(CAST((atrributed_provider_npi) AS VARCHAR), ''), COALESCE(CAST((attributed_provider_contract_id) AS VARCHAR), ''), COALESCE(CAST((member_provider_pcp_begin_date) AS VARCHAR), ''), COALESCE(CAST((managed_care_organization_contract_number) AS VARCHAR), '')), 256) AS _hash_diff,
...
{% if is_incremental() %}
  AND (SHA2_HEX(CONCAT_WS('|', COALESCE(CAST((attributed_provider_id) AS VARCHAR), ''), COALESCE(CAST((atrributed_provider_npi) AS VARCHAR), ''), COALESCE(CAST((attributed_provider_contract_id) AS VARCHAR), ''), COALESCE(CAST((member_provider_pcp_begin_date) AS VARCHAR), ''), COALESCE(CAST((managed_care_organization_contract_number) AS VARCHAR), '')), 256)) NOT IN (
