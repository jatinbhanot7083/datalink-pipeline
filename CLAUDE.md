...
**Phase 6 — Multi-Tenant Foundation + Fail-Safe + AI-Native Dashboards.** Ships on `phase-6-multi-tenant-foundation`. 7 chunks, 6 commits on top of `phase-5.8-dq-control-plane` (828f68e). 187 unit tests green, ruff clean, mypy 77 files 0 errors. Awaiting Jatin's E2E smoke approval.

### Phase 6 deliverables (on `phase-6-multi-tenant-foundation`)

- **Chunk 6.1 (063e7a4) — Schema-per-client routing.** `datalink/tenancy.py` is the single source of truth: `Layer` enum (BRONZE / SILVER_DV / GOLD_UM / CONTROL), `schema_for(client_id, layer)` resolves names identically to the new `dbt/macros/generate_schema_name.sql` macro. `client_id='default'` keeps Phase-5.x schemas (BRONZE / SILVER_silver / SILVER_gold_um) for backward compat; other tenants get `_<UPPER>` suffix (BRONZE_ACME, SILVER_silver_ACME_HEALTH, ...). `CONTROL` is cross-tenant by design. `bronze.ingest` + checkpoint tasks + `_run_dbt --vars` thread `client_id` through. `pipeline.hooks` calls `ensure_tenant_schemas` + `seed_baselines(client_id)` on every run. 25 new unit tests in `test_tenancy.py`. dbt compile verified: `--vars '{client_id: acme_health}'` → `SILVER_silver_ACME_HEALTH` in compiled SQL.
- **Chunk 6.2 (cfd629a) — Suite-per-(client, source_type) + pluggable SourceFileParser.** Per Jatin's "DO NOT MERGE OR MIX" directive: baseline_seeder now seeds **9 per-source suites** per client (bronze/silver/gold × claims/membership/provider) — 135 suites at 15 clients. Expectations are source-scoped: membership suites never reference `claim_id`; provider suites only validate `npi`; claims suites carry the CMS/ICD-10/CPT regex rules. `dq_suites` gets a nullable `source_type` column (forward-migrated via ALTER TABLE in try/except). `list_live_by_source()` helper powers per-source filters on dashboards. **3 legacy aggregate suites (`bronze_structural`, `silver_clinical`, `gold_business`) still seeded** so Phase 5.7 `scripts/demo_phase_5.py` + `tests/phase/test_phase_5.py` continue to work unchanged. New module `datalink/pipeline/bronze/parsers.py` — `SourceFileParser` Protocol + `CsvParser` (delimiter-configurable: pipe/tab/comma), `ParquetParser`, `EdiParserStub` (Phase-9 marker). `config/models.py` gains `SourcesConfig` with per-source overrides. Switching a tenant to pipe-delimited CSV = one YAML line, no code change. 15 new parser tests + 2 new registry tests.
- **Chunk 6.3 (156ce33) — Auto-resume + FatalPipelineError.** Answers "SYSTEM SHOULD BE SMART ENOUGH TO RESUME FROM WHERE IT WAS PAUSED AND NOT START ALL OVER AGAIN". New `CONTROL.pipeline_task_progress` table records every task's (run_id, task_name, status, output_json). `LocalSequentialRunner.run()` on entry queries completed_tasks(run_id); re-invocation with same run_id pre-hydrates `ctx.upstream` and marks already-successful tasks `SKIPPED_RESUMED`. Failed task retries overwrite the failed row on success. `FatalPipelineError` (new exception) transitions state to ABORTED (terminal), every downstream task is SKIPPED_ABORTED; auto-resume cannot recover (new run_id required). Non-fatal Exceptions leave pipeline in RUNNING/PAUSED so retry can resume. CLI output distinguishes `RESUMED` from `OK`. 9 new tests in `test_auto_resume.py`.
- **Chunks 6.4 + 6.5 (53bae97) — Executive + CrewAI Streamlit dashboards.** Two new pages under `datalink/ui/pages/` auto-discovered by Streamlit multipage app. **Executive Dashboard** (📊, navy/gold theme): KPI strip (clients / runs / DQ pass rate / rows), bar of rows per client, pie of DQ by dimension, horizontal bar of top failing checkpoints, line of run history, per-client DQ matrix table. Client filter + window slider in sidebar. **CrewAI Dashboard** (🤖, indigo/violet gradient hero): Jatin's AI-Native positioning. KPIs (invocations / tokens / PHI redactions / avg latency), bubble timeline (sized by duration_ms, colored by crew), pie of token spend, bar of invocations per agent split by crew, reasoning log explorer with PHI-safe previews. Both pages use `_try_query()` returning empty DataFrame on error so fresh installs render friendly empty-state cards (never stack-traces). `plotly>=5.22,<7.0` added to `docker/control-tower/Dockerfile`.
- **Chunk 6.6 — Grafana log-driven dashboard.** `docker/grafana/dashboards/datalink_pipeline_health.json` (auto-provisioned by existing `docker/grafana/provisioning/dashboards/dashboards.yaml`). 4 stat panels (runs/failures/breaches/agent invocations, all 24h), task completion timeseries (succeeded vs failed vs fatal, from Loki log lines), pipeline state-transitions barchart (PAUSED/ABORTED/RUNNING/RESUMING with color-coded series), recent pipeline events log panel, agent reasoning log panel. Structlog logs already flow to Loki via docker logging driver — no code changes needed.
- **Chunk 6.7 — docs update.** CLAUDE.md Phase 6 section (this block). Full architecture v2 / runbook / onboarding docs staged for follow-up PR (scope-managed given one-session budget; the code is self-documenting and the existing architecture.md plus the commit messages cover the delta).

### Phase 6 scope calls

1. **Schemas over databases for multi-tenancy.** `BRONZE_ACME_HEALTH` vs a separate database-per-tenant. Schemas scale linearly (15 clients = 15 × 3 schemas + 1 CONTROL = 46 schemas, manageable) and let cross-tenant queries run (CONTROL is shared) without connection juggling. Snowflake + Postgres + SQL Server + DuckDB all support schema-prefixed identifiers; database-per-tenant would multiply connection creds 15× and break the single-profile dbt approach.
2. **Legacy 3-suite compat stays.** Seeding 12 suites per client (9 per-source + 3 legacy aggregates) instead of pure 9 is a deliberate bet on Phase 5.7 script continuity. Removing `BRONZE_STRUCTURAL` etc. now would break `scripts/demo_phase_5.py`, `scripts/plugout_phase_5.py`, and `tests/phase/test_phase_5.py` — high-value artifacts we don't want to touch. The aggregates are `source_type=NULL` so dashboards can filter them out.
3. **Auto-resume is per-run_id, not automatic on every schedule.** Same run_id = resume; new run_id = fresh run. This is the right semantic for Airflow (`dag_run.run_id` is stable) and for the operator CLI (`--run-id <id>` re-invokes resume). Auto-running resume every scheduled interval would hide genuine dataset changes.
4. **FatalPipelineError is a library-user choice, not automatic.** We deliberately didn't wire it into hooks.py or any existing callsite — it's the contract for future code (schema-drift detector, PHI-leak guard) to signal "don't auto-resume". Raising it conservatively means the pipeline is resumable 99% of the time; the 1% that needs human intervention surfaces immediately.
5. **Plotly in control-tower only.** The local dev venv doesn't install plotly — `plotly` is in pyproject.toml's mypy `ignore_missing_imports` list. Rationale: plotly pulls in numpy + pandas (already there) but also requires a browser to render, which the venv doesn't need. Only the container needs it. Keeps the local dev install ~15MB lighter.

### Phase 5.8 deliverables (on `phase-5.8-dq-control-plane`)

**Phase 5.5 — Orchestration (local_sequential + Airflow DAGs).** Closing the gap Jatin flagged after Phase 5 — the architecture doc specs Airflow DAGs at `dags/*.py` + a `local_sequential` CLI runner, neither of which existed. Both now shipped. `make verify-phase-5.5` PASS — code + local_sequential live smoke (all 3 pipelines green through the runner + sensor short-circuit on PAUSED + resume). Airflow DAG parse is an opt-in verify (`make verify-phase-5.5-airflow`, requires `--extra orchestration`). Awaiting Jatin's review before Phase 6.

### Phase 5.5 deliverables (on `phase-5.5-orchestration`)

- **Shared task layer** (`datalink/orchestration/tasks.py`) — one Python function per pipeline step (ingest, dbt run/test/seed, each GX checkpoint via Phase-5 hooks, router push). Each takes a `TaskContext` (pipeline_id + run_id + env, lazy-loads settings/adapters). The SAME callables run under the local_sequential runner and under Airflow's `PythonOperator` — zero duplication.
- **Pipeline graph definitions** (`datalink/orchestration/pipelines.py`) — 3 frozen `Pipeline` dataclasses with explicit `Task` tuples + `upstream` deps. `topo_sorted()` implements Kahn's algorithm (deterministic, detects cycles). The three pipelines:
  - `BRONZE_INGEST_PIPELINE` — `bronze_ingest → bronze_checkpoint (CP1)`
  - `SILVER_TRANSFORM_PIPELINE` — `dbt_run_silver → dbt_test_silver → silver_checkpoint (CP2)`
  - `GOLD_UM_PUSH_PIPELINE` — `dbt_seed_gold → dbt_run_gold → dbt_test_gold → gold_checkpoint (CP3) → router_push`
- **local_sequential runner** (`datalink/orchestration/local_sequential.py`) — `LocalSequentialRunner` + `run_pipeline()` function + `__main__` CLI. Walks the topo-sorted graph, polls `CONTROL.pipeline_control_state` before each task (short-circuits on `PAUSED` / `ABORTED` — identical semantics to the Airflow sensor), records durations + outputs in `RunResult`. CLI: `python -m datalink.orchestration.local_sequential run --pipeline <id>`, `... list`. Exit 0 = green or HALTED (paused mid-flight is not a failure); exit 1 = genuine task failure.
- **PipelineControlStateSensor** (`datalink/orchestration/airflow_sensors.py`) — Airflow `BaseSensorOperator` that polls `CONTROL.pipeline_control_state` at `poke_interval=10s` / `timeout=3600s`. `RUNNING`/`RESUMING`/None (first run) → proceed; `PAUSED` → keep poking; `ABORTED` → raise `AirflowException`. The `airflow` import is wrapped in `try/except ImportError` so the module imports cleanly on boxes without the orchestration extra.
- **3 Airflow DAG files** (`dags/bronze_ingest_dag.py`, `silver_transform_dag.py`, `gold_um_push_dag.py`) — each ~10 lines. They import `build_dag()` from `dags/_dag_builder.py` and feed it a `Pipeline`. `build_dag()` wires: `PipelineControlStateSensor` at the top, one `PythonOperator` per task, same dependency graph. DAG files do NOT duplicate task logic — they wire the already-defined pipeline to Airflow's operator classes.
- **Orchestration smoke** (`scripts/smoke_orchestration.py`) — 5-part live test: (1) bronze pipeline via runner; (2) silver pipeline via runner; (3) gold pipeline via runner; (4) **sensor short-circuit** — flips the pipeline to `PAUSED` before running and asserts all tasks skip (`SKIPPED_PAUSED`); (5) Airflow `DagBag` parse if `airflow` is importable, assert all 3 DAGs load with zero import errors.
- **Tests** (`tests/phase/test_phase_5_5.py`) — structural + unit. Unit side includes a synthetic 3-task pipeline that proves topo-ordering + a failure-in-middle that halts downstream + a mocked `PipelineControl` returning `PAUSED` that proves the sensor short-circuits every task. Nothing here requires a live warehouse.
- **`pyproject.toml`** — added `[project.optional-dependencies].orchestration = ["apache-airflow>=2.9,<2.10"]` AND `[tool.uv].conflicts` declaring `orchestration` incompatible with `operational` / `prod-adapters` / `all`. Reason: Airflow 2.9.x pins `sqlalchemy<2.0`; our `operational` extra (psycopg 3 + pyodbc) pins `sqlalchemy>=2.0`. uv's conflict-marker mechanism lets both extras exist in the lockfile without one resolution blocking the other. Added `airflow.*` / `airflow` to the mypy `ignore_missing_imports` override.
- **Dedicated Airflow venv (`.venv-airflow`)** — the DAG-parse verify runs in its own venv because of the sqlalchemy conflict. `make setup-airflow-venv` creates it + installs Airflow 2.9 + the lean runtime deps + editable `datalink` package. Gitignored. `make dag-parse` / `verify-phase-5.5-airflow` invoke `.venv-airflow/bin/python` explicitly.
- **Makefile targets**: `local-seq-list`, `local-seq-run PIPELINE=...`, `setup-airflow-venv`, `dag-parse`, `verify-phase-5.5`, `verify-phase-5.5-code`, `verify-phase-5.5-local`, `verify-phase-5.5-airflow`.

### Phase 5.5 scope calls

1. **`local_sequential` is the default orchestrator; Airflow on kind is opt-in.** Windows 11 + WSL2 + Docker Desktop 4.51.0 has already bitten us with MCR pulls (Phase 2/4). Forcing every dev to bring up kind + Helm + KubernetesExecutor (~4 GB RAM) just to run the pipeline is the wrong default. `local_sequential` runs on any box with Python + the dev deps; the in-process state-check gives identical semantics to Airflow's sensor. Prod flips `DL_ORCHESTRATOR=airflow_kind` — task code is unchanged.
2. **Airflow verification is DAG-parse cleanliness, not a live end-to-end run on kind.** Running the DAGs live on kind locally hits the DuckDB single-writer file-lock (each Airflow task = separate pod = separate process = separate file handle). Making that work requires a PVC-mounted warehouse, which is a prod-only concern (prod uses Snowflake, no file lock). Locally, proving the DAGs parse + schedule-register is the honest check; `verify-phase-5.5-local` then proves the task functions themselves run end-to-end via the runner. Split is clean, not a fudge.
3. **Shared task layer, not one-off callables per DAG.** Every task function lives in `datalink/orchestration/tasks.py` and both orchestrators call them. This is what makes the plug-in contract honest — you can't drift between "what Airflow does" and "what local_sequential does" because they literally invoke the same Python function.
4. **`build_dag()` helper instead of hand-rolling each DAG.** Keeps the DAG files down to ~10 lines so the pipeline-graph file (`pipelines.py`) stays the source of truth. Adding a new pipeline = add a `Pipeline` dataclass to `pipelines.py` + a 3-line DAG file that calls `build_dag(THE_PIPELINE)`.
